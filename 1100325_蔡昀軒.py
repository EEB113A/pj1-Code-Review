...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        # 2.判斷左右是否連線
        # <<<將你的程式寫在這邊>>>
        for y in range(10):      #垂直y軸的0~9列(判斷左右時與y無關,所以範圍一樣0~9列不變)
            for x in range(1,7): #垂直x軸的1~6行,扣除第0行和第7行(無法與最左及最右比較形成三個相同貓咪,會超出邊界)
                if self.check[y][x] > 0:
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]: #選定之xy的左邊(x-1)和右邊(x+1)三者相同時
                        self.neko[y][x-1] = 7  #變為肉球(代號7),即消除
                        self.neko[y][x] = 7    #同上
                        self.neko[y][x+1] = 7  #同上
        # 3.判斷斜角是否連線
        # <<<將你的程式寫在這邊>>>
        for y in range(1,9):     #垂直y軸的1~8列,扣除第0列和第9列(無法與左上左下或右上右下比較形成三個相同貓咪,會超出邊界)
            for x in range(1,7): #垂直x軸的1~6行,扣除第0行和第7行(無法與左上左下或右上右下比較形成三個相同貓咪,會超出邊界)
                if self.check[y][x] > 0:
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]: #選定之xy的左上(x-1 y-1)和右下(x+1 y+1)三者相同時
                        self.neko[y-1][x-1] = 7  #變為肉球(代號7),即消除
                        self.neko[y][x] = 7      #同上
                        self.neko[y+1][x+1] = 7  #同上
                    if self.check[y+1][x-1] == self.check[y][x] and self.check[y-1][x+1] == self.check[y][x]: #選定之xy的左下(x-1 y+1)和右上(x+1 y-1)三者相同時
                        self.neko[y+1][x-1] = 7  #變為肉球(代號7),即消除
                        self.neko[y][x] = 7      #同上
                        self.neko[y-1][x+1] = 7  #同上
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        TrueorFlase = True        #先假設全部貓咪都可以落下完畢
        for y in range(8,-1,-1):  #從第8列開始判斷空格(第9列已有第一次下去的貓咪),如果有空格,貓咪可以往下一格,接著再往上判斷(-1的部分),如果有空格,貓咪即可再往下,直到無法再下去
            for x in range(8):
                if self.neko[y][x] > 0:
                    if self.neko[y+1][x] == 0:  #貓咪下方有空格([y+1][x]==0)
                        TrueorFlase = False     #Flase表示未落下完畢,所以可以繼續落下
                        self.neko[y+1][x] = self.neko[y][x]  #原本位在xy的貓咪落下一格,變位在x y+1
                        self.neko[y][x] = 0  #原本位在xy的貓咪落下後,原本xy的位置變為空白(0)
        return TrueorFlase  #回傳是否已經落下完畢
#========================================================= Question 3 =========================================================
    # 題目說明：
    ...
    def over_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for x in range(8):
            if self.neko[0][x] > 0:  #[0]為y的第0列(最上層), >0表示無空白(碰到最上層了)
                return True          #碰到最上層回傳True
        return False
#==============================================================================================================================
newgame = CatGame()
newgame.run()