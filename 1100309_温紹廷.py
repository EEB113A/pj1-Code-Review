            ...
            for y in range(9):
                if(self.neko[y][x])>0:
                   if (self.neko[y+1][x])==0:
                       ...
                       self.neko[y][x]=0
                       break                                  #從上往下判斷是否下面的那格為0，不為0在往下掉
        for xx in range(8):
            for yy in range(1,10):
                if self.neko[yy][xx]==0:
                    if self.neko[yy-1][xx]!=0:
                        down=False                            #如果有一格為0但上面不是0表示還沒下落完
        return down
