...
class CatGame:
    ...
    def game_main(self):
        ...
        # step5 滑鼠點擊配置貓咪方塊
            ...
            if 24 <= self.mouse_x and self.mouse_x < 24+72*8 and 24 <= self.mouse_y and self.mouse_y < 24+72*10:
                ...
                if self.mouse_click == 1:       # 當滑鼠被點擊時
                    ...
                    self.set_neko()
                    # 在滑鼠的位置, 隨機產生一個貓咪方塊
                    self.neko[self.cursor_y][self.cursor_x] = self.cat
                    self.cat = 0
                    self.step = 2
            self.canvas.delete("CURSOR")
            self.canvas.create_image(
                self.cursor_x*72+60, self.cursor_y*72+60, image=self.cursor, tag="CURSOR")
            self.draw_neko()
        ...
        # step6 處理遊戲結束
            ...
            self.timer += 1
            if self.timer == 1:
                self.draw_txt("GAME OVER", 312, 348, 68, "red",
                              "OVER")      # 顯示 GAME OVER
            if self.timer == 50:
                self.canvas.delete("OVER")
                self.step = 0
        self.canvas.delete("INFO")
        self.draw_txt("SCORE " + str(self.score), 160,
                      60, 32, "blue", "INFO")      # 顯示分數
        if self.cat > 0:      # 假設下一個要配置的貓咪
            self.canvas.create_image(
                752, 128, image=self.img_neko[self.cat], tag="INFO")      # 顯示該貓咪方塊
    ...
    def draw_txt(self, txt, x, y, siz, col, tg):
        ...
        self.canvas.create_text(
            x+2, y+2, text=txt, fill="black", font=fnt, tag=tg)
        ...
        for y in range(10):
            for x in range(8):
                if self.neko[y][x] > 0:       # 如果neko陣列中大於 0, 則顯示對應的貓咪圖片
                    # (x*72+60, y*72+60) 代表指定了格子中心點的座標 !
                    self.canvas.create_image(
                        x*72+60, y*72+60, image=self.img_neko[self.neko[y][x]], tag="NEKO")
...
# ========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        #     for y in range(9):
        #         for x in range(8):
        #             if self.neko[y][x] > 0 and self.neko[y+1][x] == 0:
        #                 self.neko[y+1][x] = self.neko[y][x]
        #                 self.neko[y][x] = 0
        # return True

        for x in range(8):
            if self.neko[self.count_row][x] > 0 and self.neko[self.count_row + 1][x] == 0:
                self.neko[self.count_row + 1][x] = self.neko[self.count_row][x]
                self.neko[self.count_row][x] = 0
