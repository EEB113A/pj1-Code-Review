...
class CatGame:
    def __init__(self):
        self.root = tk.Tk()     # 創建UI介面視窗的物件
        self.step = 0           # 管理遊戲流程，有 0~6 七個流程需要處理，一開始設成 0 代表從 0 開始
        self.timer = 0          # 管理時間
        self.score = 0          # 管理分數
        self.cat = 0            # 下一個貓咪變數
        ...
        self.neko = []          # 貓咪矩陣，處理每一格貓咪顏色的狀況，儲存貓咪棋盤格的參數
        self.check = []         # 確認是否相連的矩陣
        for i in range(10):  # 都是 10x10 的矩陣
    ...
    # 遊戲主程式
    def game_main(self):  # 在每次執行時會先確認 self.step 值為多少再去執行對應 if 的區塊
        # step0 處理顯示標題
        if self.step == 0:
        ...
        # step2 處理貓咪方塊落下
            ...
            if self.drop_neko() == True:
                self.step = 3
            self.draw_neko()  # 直接跑到下面區塊去執行程式碼，因為 step 值是 2 所以會一直會到這部分直到回傳 True 值
            # 等於是會一直落下到底部才會去處理連線
        ...
        self.root.after(100, self.game_main)  # after()：在多久之後再執行某個函式，這邊單位是毫秒，所以 100 毫秒（ 0.1 秒）
        # 每 0.1 秒去執行遊戲主程式一次，讓他去做對應的處理，才顯示成好像動畫一樣
...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for x in range(1,7): 
            for y in range(10):
                if self.check[y][x] > 0:  # 假如不是空的開始跑回圈
