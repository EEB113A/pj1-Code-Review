...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(10): 
            for x in range(1,7):
                if self.check[y][x] > 0:
                    if self.check[y][x+1] == self.check[y][x] and self.check[y][x] == self.check[y][x-1]:
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y][x+1] = 7
        # 3.判斷斜角是否連線
        # <<<將你的程式寫在這邊>>>
        for y in range(1,9):  #判斷右上到左下斜角
            for x in range(1,7):
                if self.check[y][x] > 0:
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for i in range(8,-1,-1): #判斷每個方塊下方是否還有空間可落下(若有則下落一格)
            for j in range(8):
                if self.neko[i+1][j]==0:
                    self.neko[i+1][j]=self.neko[i][j]
                    self.neko[i][j]=0
    
        for j in range(8): #判斷方塊是否落到能落到的最底層(沒有則回傳False)
            if self.neko[8][j]!=0 and self.neko[9][j]==0:
                return False   
            if self.neko[7][j]!=0 and self.neko[8][j]==0:
                return False
            if self.neko[6][j]!=0 and self.neko[7][j]==0:
                return False
            if self.neko[5][j]!=0 and self.neko[6][j]==0:
                return False
            if self.neko[4][j]!=0 and self.neko[5][j]==0:
                return False
            if self.neko[3][j]!=0 and self.neko[4][j]==0:
                return False
            if self.neko[2][j]!=0 and self.neko[3][j]==0:
                return False
            if self.neko[1][j]!=0 and self.neko[2][j]==0:
                return False
            if self.neko[0][j]!=0 and self.neko[1][j]==0:
                return False
        return True #落到最底層後回傳True
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        # hint : 此方法需要回傳值(return)，型態為布林值(bool : True, False)
        # <<<將你的程式寫在這邊>>>
        if self.neko[0][0]!=0 or self.neko[0][1]!=0 or self.neko[0][2]!=0 or self.neko[0][3]!=0 or self.neko[0][4]!=0 or\
           self.neko[0][5]!=0 or self.neko[0][6]!=0 or self.neko[0][7]!=0 : #碰到最上層的情況
            return True 
        else:
            return False
#==============================================================================================================================
