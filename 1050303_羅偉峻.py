...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
            ...
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:
                        self.neko[y][x-1] = 7                      #依照教授給的上下消除，把座標改成左右
                        self.neko[y][x] = 7
            ...
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y - 1][x - 1] == self.check[y][x] and self.check[y + 1][x + 1] == self.check[y][x]:
                        self.neko[y - 1][x - 1] = 7            #依照教授給的上下消除，把座標改成右上左下斜線
                        self.neko[y][x] = 7
        ...
        for y in range(1, 9):
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y - 1][x + 1] == self.check[y][x] and self.check[y + 1][x - 1] == self.check[y][x]:
                        self.neko[y - 1][x + 1] = 7              #依照教授給的上下消除，把座標改成左上右下斜線
                        self.neko[y][x] = 7
                        self.neko[y + 1][x - 1] = 7  # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
    ...
    # <<<將你的程式寫在這邊>>>
        has_drop = False                                        # 先預設為false
        for i in reversed(range(9)):                            # 從右下角開始檢查
            for x in reversed(range(8)):
                neko = self.neko[i][x]
                if neko > 0 and self.neko[i + 1][x] == 0:        # 若下方格子是空的就往下一格
                    self.neko[i][x] = 0
                    self.neko[i + 1][x] = neko
                    has_drop = True
        if has_drop:
            return False
        else:
    ...
    # <<<將你的程式寫在這邊>>>
        for x in range(8):
            check_all_neko = False             # 先設定沒有碰到坐上層
            for i in range(10):                # 一行一行檢查
                neko = self.neko[i][x]
                if neko == 0:                  # 如果這行有空格就break
                    break
                elif i == 9:                   #如果沒有空格就代表碰到坐上層，就return True
                    check_all_neko = True
            if check_all_neko:
                return True
        return False
