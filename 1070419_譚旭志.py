...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(10): 
            for x in range(1, 7):
                if self.check[y][x] > 0:
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:    #用x項的+1-1項判斷左右
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y][x+1] = 7
        ...
        for y in range(1, 9): 
            for x in range(1, 7):
                if self.check[y][x] > 0:
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]:    #判斷左下右上的斜線
                        self.neko[y-1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x+1] = 7
                    elif self.check[y-1][x+1] == self.check[y][x] and self.check[y+1][x-1] == self.check[y][x]:    #判斷右下左上的斜線
                        self.neko[y-1][x+1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x-1] = 7
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        for y in range(9):
            for x in range(8):
                if self.neko[8-y+1][7-x] == 0 and self.neko[8-y][7-x] > 0:    #從右下往左上去判斷是否有下面一格是空格的貓咪方塊
                    self.neko[8-y+1][7-x] = self.neko[8-y][7-x]    #上面的貓咪方塊複製到下面的空格
                    self.neko[8-y][7-x] = 0    #消除原本上面的貓咪方塊
                    drop_check = False    #如果有進入此迴圈表示貓咪方塊還沒全部降下完成，要return False
        return drop_check
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        top_check = False
        for x in range(8):
            if self.neko[0][x] > 0:    #如果最上面一排有東西就進入迴圈
                top_check = True    #回傳為True並終止迴圈
                break
        return top_check
