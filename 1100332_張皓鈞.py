        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    ...
                    # 2.判斷左右是否連線
                    elif self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        check_gameover = [0,0,0,0,0,0,0,0]
        for y in range(10):
            for x in range(8):
                if self.neko[y][x] > 0:
                    check_gameover[x] += 1
        if 10 in check_gameover:
            return True
        else:
            return False
        # hint : 此方法需要回傳值(return)，型態為布林值(bool : True, False)
