...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(0,10): 
            for x in range(8):
                if self.check[y][x] > 0: 
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:  #確認貓咪左右邊是否為同一貓咪顏色
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y][x+1] = 7 
        # 3.判斷斜角是否連線
        ...
        for y in range(1,9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]:  #確認貓咪斜邊(左上到右下)是否為同一貓咪顏色
                        self.neko[y-1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x+1] = 7
                    if self.check[y+1][x-1] == self.check[y][x] and self.check[y-1][x+1] == self.check[y][x]:  #確認貓咪斜邊(右上到左下)是否為同一貓咪顏色
                        self.neko[y+1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y-1][x+1] = 7 
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for y in range(8,-1,-1):  #迴圈由下往上一列一列測試
            for x in range(8):
                if self.neko[y+1][x]==0:  #如果貓咪方塊下面一格沒有東西
                    self.neko[y+1][x]=self.neko[y][x]  #將貓咪方塊與下一格調換
                    self.neko[y][x]=0  #貓咪方塊變回空白格

                check = 0  #設一變數為0
                if y == 0 and x == 7:  #當迴圈跑完最後一格
                    for y in range(9):
                        for x in range(8):
                            if self.neko[y+1][x]==0 and self.neko[y][x]!=0:  #檢視貓咪方塊是否到達最底部且上方沒有方塊卡在半空中
                                check += 1  #如果有任何一個方塊沒到達最底或卡在半空中，變數加一
                    if check == 0:  #當變數為0時，代表貓咪方塊到達最底部且上方沒有方塊卡在半空中
                        return True  #進行下一個步驟
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        total =0  #設一個變數為0
        for x in range(8):
            total += self.neko[y][x] #如果最上面一列有貓咪方塊，變數加一
        if total == 0:  #若沒有任何貓咪方塊到達最上面
            return False  #遊戲繼續
        else:  #若有貓咪方塊到達最上面一列
            return True  #遊戲結束
