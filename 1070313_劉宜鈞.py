...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        # 2.判斷左右是否連線
        # <<<將你的程式寫在這邊>>>
        for x in range(1,7): # x軸的格數
            for y in range(10): # y軸的格數
                if self.check[y][x] > 0: # 如果大於0就開始跑進迴圈裡
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]: # 看看左右有無一樣
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y][x+1] = 7 
                        # 7 => 把位置上的圖片換掉，相連後讓圖片消掉
        ...
        for y in range(1,9): 
            for x in range(1,7):
                if self.check[y][x] > 0:
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]: # 看看右下到左上有無一樣
                        self.neko[y-1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x+1] = 7 
                        # 7 => 把位置上的圖片換掉，相連後讓圖片消掉

                    elif self.check[y-1][x+1] == self.check[y][x] and self.check[y+1][x-1] == self.check[y][x]: # 看看右下到左上有無一樣 
                        self.neko[y-1][x+1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x-1] = 7 
                        # 7 => 把位置上的圖片換掉，相連後讓圖片消掉
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        dropch=True  #先設回傳值是True
        for y in range(9): # y軸
            for x in range(8): # x軸
                if self.neko[y][x]>0 and self.neko[y+1][x]==0: # 如果方塊落下有值(有東西)，而且下一格是0，進入迴圈
                    self.neko[y+1][x]=self.neko[y][x] # 方塊落下
                    self.neko[y][x]=0 # 把原格子變0(清掉) 
                    dropch=False 
        


        return dropch # 跑完迴圈 return True
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        y=0
        check=False # 先設回傳值是False
        for x in range(8):
            if self.neko[y][x] > 0: # 如果大於0就開始跑進迴圈裡
                check=True
                break 
        # check變成True時，終止迴圈
