...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        # 2.判斷左右是否連線
        # <<<將你的程式寫在這邊>>>
        for x in range(1,7):    #先設定x跑1~(7-1)接著在設定y跑0到9,假如不是空的（初始值）就開始跑for loop
            for y in range(0,10):     
                if self.check[y][x]>0: 
                    #下方先判斷是否為相同
                    if  self.check[y][x-1]==self.check[y][x] and self.check[y][x+1]==self.check[y][x]:
                            self.neko[y][x-1]=7   #將貓貓[y][x-1]的位置換成另個圖片顯示,表示已經被消除
                            self.neko[y][x]=7     #將貓貓[y][x]的位置換成另個圖片顯示,表示已經被消除
                            self.neko[y][x+1]=7   #將貓貓[y][x+1]的位置換成另個圖片顯示,表示已經被消除
        # 3.判斷斜角是否連線
        # <<<將你的程式寫在這邊>>>
        for y in range(1,9):    #先設定y跑1~(9-1)接著在設定x跑1~(7-1),假如不是空的（初始值）就開始跑for loop
            for x in range(1,7):
                if self.check[y][x]>0:
                    #下方先判斷右下到左上是否為相同
                    if  self.check[y-1][x-1]==self.check[y][x] and self.check[y+1][x+1]==self.check[y][x]:
                            self.neko[y-1][x-1]=7   #將貓貓[y-1][x-1]的位置換成另個圖片顯示,表示已經被消除
                            self.neko[y][x]=7       #將貓貓[y][x]的位置換成另個圖片顯示,表示已經被消除
                            self.neko[y+1][x+1]=7   #將貓貓[y+1][x+1]的位置換成另個圖片顯示,表示已經被消除
                    #下方接著用else-if判斷左下到右上是否為相同
                    elif  self.check[y-1][x+1]==self.check[y][x] and self.check[y+1][x-1]==self.check[y][x]:
                                self.neko[y-1][x+1]=7   #將貓貓[y-1][x+1]的位置換成另個圖片顯示,表示已經被消除
                                self.neko[y][x]=7       #將貓貓[y][x]的位置換成另個圖片顯示,表示已經被消除
                                self.neko[y+1][x-1]=7   #將貓貓[y+1][x-11]的位置換成另個圖片顯示,表示已經被消除
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        drop_val=True #先假設drop value的回傳值是true
        for y in range(0,9):    #for loop先跑y從0~(9-1),x從0~(8-1)
            for x in range(0,8):
                if  self.neko[y+1][x]==0 and self.neko[y][x]>0: #先判斷是否為初始值並且是否有要往下
                        self.neko[y+1][x]=self.neko[y][x]   #y座標將會往下
                        self.neko[y][x]=0   #將原本的位置初始
                        drop_val=False  #此loop還未跑完，因次先給他False讓他繼續跑
        


        return drop_val  #將drop value回傳回去
#========================================================= Question 3 =========================================================
    # 題目說明：
    # 1.判斷貓咪方塊是否碰到到最上層，是則回傳 True，否則回傳 False。
    def over_neko(self):
        y=0 #先初始化y值
        check=False #先假設check為False
        for x in range(0,8): #先設定for loop的x從0~(8-1)
            if self.neko[y][x]>0:   #判斷是否為空的
                check=True  #再將check設為True
                break  #當偵測到碰到最頂部,就break跳出此loop

        return check    #回傳check
        # hint : 此方法需要回傳值(return)，型態為布林值(bool : True, False)
        # <<<將你的程式寫在這邊>>>
