...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        for i in range(9):
            for j in range(8):
                if self.neko[i][j] !=0 and i != 10 :
                    if self.neko[i+1][j] == 0:
                        b.append([i,j])
        for i in range(len(b)):
            self.neko[b[i][0] + 1][b[i][1]] = self.neko[b[i][0]][b[i][1]]
            self.neko[b[i][0]][b[i][1]] = 0
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        # hint : 此方法需要回傳值(return)，型態為布林值(bool : True, False)
        # <<<將你的程式寫在這邊>>>
        self.listcount=[1,1,1,1,1,1,1,1]
        for y in range(9):
            for x in range(7):
                if self.neko[y][x]>0:
                    self.listcount[x]+=1#Y軸每有一個貓咪圖案就+1
        if 10 in self.listcount:#若其中一排有十個就回傳true
            return True
        else:
            return False
