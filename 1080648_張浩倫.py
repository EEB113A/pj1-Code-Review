...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        # 2.判斷左右是否連線
        # <<<將你的程式寫在這邊>>>
        for y in range(1, 10):#縱軸有10格 第零格不算 
            for x in range(8):#橫軸有8格
                if self.check[y][x] > 0:#如果那格有貓咪
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:#如果那格的左右邊都一樣
                        self.neko[y][x-1] = 7# 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
                        self.neko[y][x] = 7# 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
                        self.neko[y][x+1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
        

        # 3.判斷斜角是否連線
        # <<<將你的程式寫在這邊>>>
        for y in range(1, 9):#縱軸有8格 第零格不算 
            for x in range(8):#橫軸有8格
                if self.check[y][x] > 0:#如果那格有貓咪
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]:#如果那格的左下到右上都一樣
                        self.neko[y-1][x-1] = 7# 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
                        self.neko[y][x] = 7# 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
                        self.neko[y+1][x+1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
                    if self.check[y+1][x-1] == self.check[y][x] and self.check[y-1][x+1] == self.check[y][x]:#如果那格的右下到左上都一樣
                        self.neko[y+1][x-1] = 7# 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
                        self.neko[y][x] = 7# 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
                        self.neko[y-1][x+1] = 7# 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        #此處self.count代表y值
        for x in range(8):#判斷橫軸的每個值
            if self.neko[self.count][x] > 0 and self.neko[self.count + 1][x] == 0:#如果那格有貓咪且下面是空的
                self.neko[self.count + 1][x] = self.neko[self.count][x]#往下移一格
                self.neko[self.count][x] = 0#把原位置變成空的
     
        if self.count < 8:#如果y值還沒到最下面
            self.count = self.count + 1  #y值+1
        else: 
            self.count=0 #y歸零
            for x in range(8):#判斷橫軸的每個值
                for i in range(8):#重複判斷8次
                    for y in range(9):#設y從0到8時
                        if self.neko[y][x] > 0 and self.neko[y + 1][x] == 0:#如果那格有貓咪且下面是空的
                            self.neko[y + 1][x] = self.neko[y][x]#往下移一格
                            self.neko[y][x] = 0#把原位置變成空的
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        if self.neko[0][0] > 0 or self.neko[0][1] > 0 or self.neko[0][2] > 0 or self.neko[0][3] > 0 or self.neko[0][4] > 0 or self.neko[0][5] > 0 or self.neko[0][6] > 0 or self.neko[0][7] > 0:
            return True#當最上面那排有貓咪時 回傳true
        else:#否則  
