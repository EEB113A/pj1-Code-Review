    ...
    def game_main(self):
        ...
        # step5 滑鼠點擊配置貓咪方塊
            ...
            if 24 <= self.mouse_x and self.mouse_x < 24+72*8 and 24 <= self.mouse_y and self.mouse_y < 24+72*10:
                ...
                #while self.neko[self.cursor_y][self.cursor_x]==0:
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        for y in range(9,0,-1):  #由下至上檢查是否交換(落下)
            for x in range(8):
                for i in range(10-y):
                    if self.neko[y-1][x]!=0 and self.neko[y][x]==0:
                        self.neko[y-1][x],self.neko[y][x]=self.neko[y][x],self.neko[y-1][x]  #若方塊的下一個為0就交換
                        return False
        
        if self.neko[y+i][x]!=0:  
            return True
#========================================================= Question 3 =========================================================
    # 題目說明：
    ...
    def over_neko(self):
        ...
        for x in range(8): 
            if self.neko[0][x] != 0 and self.neko[1][x] != 0 :  #若最上層有方塊且下一層不為0則結束遊戲
                return True  
