...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(0, 10): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:#檢查左右相鄰的格子是否有圖案
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
        ...
        for y in range(1,9):
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y-1][x+1] == self.check[y][x] and self.check[y+1][x-1] == self.check[y][x]:#檢查斜對角的格子是否有圖案
                        self.neko[y-1][x+1] = 7
                        self.neko[y][x] = 7
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        flag = True
        for y in range(9,0,-1):#由下到上，檢查上層是否有圖案
            for x in range(0,8):
                if self.neko[y][x] == 0:#如果下格沒有圖案，替補上格的圖案
