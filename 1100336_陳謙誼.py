...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(0,10): 
            for x in range(9):
                if self.check[y][x] > 0:
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:  # 以self.neko[y][x]為基準，判斷左右是否同數字(貓咪代號) 
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
        ...
        for y in range(1,9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]:  # 以self.neko[y][x]為基準，判斷左上右下是否同數字(貓咪代號) 
                        self.neko[y-1][x-1] = 7
                        self.neko[y][x] = 7
        ...
        for y in range(1,9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y+1][x-1] == self.check[y][x] and self.check[y-1][x+1] == self.check[y][x]:  # 以self.neko[y][x]為基準，判斷左下右上是否同數字(貓咪代號) 
                        self.neko[y+1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y-1][x+1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        for x in range(8): 
            for y in range(9):
                if self.neko[y][x]!=0 and self.neko[y+1][x]==0:  #如果self.neko[y][x]的直不為0，且下面一格的數字為0
                    self.neko[y+1][x]=self.neko[y][x]  #下面那格的數字變成上面那格的數字
                    self.neko[y][x]*=0 #原本的那格歸0

                    if x==7 and y==8:
                        return True
                else:
                    y+=1
#                elif self.neko[y][x]==0: #如果self.neko[y][x]為0
#                    y+=1  #繼續向下檢查是否貓咪有到底

                
#                for i in range(9,0,-1):
#                    if self.neko[y][x]!=0 and self.neko[i-1][x]==0:
#                        flag=True
#                    else:
#                        flag=False
