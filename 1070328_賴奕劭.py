...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(10): 
            for x in range(1, 7):
                if self.check[y][x] > 0:
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:       # 判斷左右是否與自己相等
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y][x+1] = 7    # 消除
        ...
        for y in range(1, 9): 
            for x in range(1, 7):
                if self.check[y][x] > 0:
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]:   # 判斷左下及右上是否與自己相等
                        self.neko[y-1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x+1] = 7     # 消除
                    elif self.check[y-1][x+1] == self.check[y][x] and self.check[y+1][x-1] == self.check[y][x]:    # 判斷右下及左上是否與自己相等
                        self.neko[y-1][x+1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x-1] = 7     # 消除
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        for y in range(9):
            for x in range(8):
                if self.neko[8-y+1][7-x] == 0 and self.neko[8-y][7-x] > 0:      # 若自己上面那格沒有方塊以及自己那格有方塊
                    self.neko[8-y+1][7-x] = self.neko[8-y][7-x]
                    self.neko[8-y][7-x] = 0                                     # 自己上方那格及自己那格皆沒有方塊
                    drop_check = False                                          # 回傳False
        return drop_check
#========================================================= Question 3 =========================================================
    # 題目說明：
    ...
    def over_neko(self):
        # hint : 此方法需要回傳值(return)，型態為布林值(bool : True, False)
        # <<<將你的程式寫在這邊>>>
        top_check = False                       # 初始將top_check預設為False
        for x in range(8):
            if self.neko[0][x] > 0:             # 判斷最上層是否有貓咪方塊
                top_check = True
                break
        return top_check
